...
def plot_enrichment_summary(
    ...
    negatome_vals = [
        r.negatome_control_ratio for r in results if r.negatome_control_ratio is not None
    ]
    if negatome_vals:
        fig.add_trace(
            go.Bar(
                x=[
                    labels[i] for i, r in enumerate(results) if r.negatome_control_ratio is not None
                ],
                y=negatome_vals,
