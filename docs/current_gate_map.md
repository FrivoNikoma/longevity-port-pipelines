...
`data/input/g3sx30_one_row_first_minimal_controlled_downstream_outputs.csv` creates the first minimal controlled downstream output for the one-row ready G3SX30 elephant MDM2 artifact. This is an actual output record, not another non-result layer.

The output sources `data/input/g3sx30_one_row_controlled_downstream_read_check_results.csv#1`, with `source_read_check_status=controlled_downstream_read_check_passed`, `source_controlled_handle_id=g3sx30_elephant_mdm2_one_row_ready_artifact_controlled_downstream_handle`, `source_controlled_input_status=one_row_ready_artifact_available_for_controlled_downstream_use`, and `source_ready_artifact_reference=data/output/embeddings/esmc-300m-2024-12/tp53_mdm2_elephant_seed_mdm2_chain_mdm2_9785.npy`.

The output is `output_action=add_first_minimal_controlled_downstream_output_for_one_row_ready_g3sx30_artifact`, `output_status=first_minimal_controlled_downstream_output_created`, `output_type=one_row_artifact_identity_and_embedding_health_summary`, `output_scope=identity_and_embedding_health_only_no_biological_claim`, `candidate_id=tp53_mdm2_elephant_seed_mdm2_chain`, `target_accession=G3SX30`, `target_accession_db=UniProtKB TrEMBL`, `target_species=Loxodonta africana`, `target_taxid=9785`, `gene_symbol=MDM2`, `one_row_only=true`, and `ready_scope=one_row_g3sx30_elephant_mdm2_only`.

The output confirms `candidate_identity_confirmed=true`, `artifact_reference_confirmed=true`, `embedding_health_confirmed=true`, `source_embedding_shape=492x960`, `source_embedding_dtype=float32`, `source_embedding_finite=true`, `source_sequence_length=492`, and `source_sequence_length_matches=true`.

This output keeps `gate8_promoted=false`, `gate9_promoted=false`, and `biological_claim_made=false`. It does not make a Biohub / ESMC call, does not rerun live embedding, does not generate a new embedding, does not commit the generated `.npy` artifact, does not commit any `data/output` artifact, does not promote Gate 8 or Gate 9, does not call Boltz / AF3 / Chai, does not rerun enrichment or contrast, and does not make a biological claim.

After this PR, the next step should be a concrete analysis-adjacent controlled output or another concrete biological-data-bearing step: `move_toward_first_analysis_adjacent_controlled_output_or_next_concrete_biological_data_bearing_step_for_one_row_ready_g3sx30_artifact`.

The next-step guard fields are `next_pr_should_be_concrete_analysis_adjacent_or_biological_data_bearing_step=true` and `no_additional_non_result_layer_before_next_concrete_step=true`. Do not insert another non-result layer before that concrete step.

## G3SX30 one-row controlled downstream read/check result

`data/input/g3sx30_one_row_controlled_downstream_read_check_results.csv` records the actual controlled downstream read/check result for the one-row ready G3SX30 elephant MDM2 artifact. This is not another approval layer, scaffold, handle, or decision layer.

The read/check result sources `data/input/g3sx30_one_row_controlled_downstream_use_paths.csv#1`, with `source_controlled_downstream_use_path=first_controlled_downstream_use_path_for_one_row_ready_artifact`, `source_controlled_handle_id=g3sx30_elephant_mdm2_one_row_ready_artifact_controlled_downstream_handle`, `source_controlled_input_status=one_row_ready_artifact_available_for_controlled_downstream_use`, `source_ready_artifact_reference=data/output/embeddings/esmc-300m-2024-12/tp53_mdm2_elephant_seed_mdm2_chain_mdm2_9785.npy`, `source_next_pr_must_be_actual_controlled_downstream_read_check=true`, and `source_no_additional_downstream_approval_before_read_check=true`.

The controlled read/check result is `read_check_action=run_controlled_downstream_read_check_for_one_row_ready_g3sx30_artifact`, `read_check_status=controlled_downstream_read_check_passed`, `controlled_handle_id=g3sx30_elephant_mdm2_one_row_ready_artifact_controlled_downstream_handle`, `controlled_input_status=one_row_ready_artifact_available_for_controlled_downstream_use`, `one_row_only=true`, `candidate_id=tp53_mdm2_elephant_seed_mdm2_chain`, `target_accession=G3SX30`, `target_accession_db=UniProtKB TrEMBL`, `target_species=Loxodonta africana`, `target_taxid=9785`, `gene_symbol=MDM2`, and `ready_scope=one_row_g3sx30_elephant_mdm2_only`.

The local artifact read/check recorded `local_runtime_embedding_exists=true`, `local_runtime_embedding_tracked=false`, `local_runtime_embedding_committed=false`, `git_ignore_rule_status=data_output_ignored`, `embedding_shape=492x960`, `embedding_dtype=float32`, `embedding_finite=true`, `sequence_length=492`, and `sequence_length_matches=true`.

This read/check keeps `gate8_promoted=false`, `gate9_promoted=false`, and `biological_claim_made=false`. It does not make a Biohub / ESMC call, does not rerun live embedding, does not generate a new embedding, does not commit the generated `.npy` artifact, does not commit any `data/output` artifact, does not promote Gate 8 or Gate 9, does not call Boltz / AF3 / Chai, does not rerun enrichment or contrast, and does not make a biological claim.

After this PR, the next step should move toward the first minimal controlled downstream output: `move_toward_first_minimal_controlled_downstream_output_for_one_row_ready_g3sx30_artifact`.

The next-step guard fields are `next_pr_should_move_toward_first_minimal_controlled_downstream_output=true` and `no_additional_read_check_approval_before_output=true`. Do not add another read/check approval, review, scaffold, handle, or decision layer before that output-oriented step.
...
The transition result sources `data/input/g3sx30_one_row_readiness_preflight_transition_decisions.csv#1`, with `source_decision=approve_one_row_readiness_preflight_transition_path`, `source_approved_for_next_transition_step=true`, `source_allowed_next_action=run_one_row_g3sx30_readiness_preflight_transition`, `source_next_pr_must_be_actual_transition_check=true`, and `source_no_additional_decision_before_transition=true`.

The transition result is `transition_action=run_one_row_g3sx30_readiness_preflight_transition`, `transition_status=one_row_readiness_preflight_transition_passed`, `one_row_only=true`, `candidate_id=tp53_mdm2_elephant_seed_mdm2_chain`, `target_accession=G3SX30`, `target_accession_db=UniProtKB TrEMBL`, `target_species=Loxodonta africana`, `target_taxid=9785`, `gene_symbol=MDM2`, `ready_for_preflight=true`, and `ready_scope=one_row_g3sx30_elephant_mdm2_only`.

The `ready_for_preflight=true` status applies only to this one row. This transition/check result keeps `gate8_promoted=false`, `gate9_promoted=false`, and `biological_claim_made=false`.

This transition/check result does not make a Biohub / ESMC call, does not rerun live embedding, does not generate a new embedding, does not commit the generated `.npy` artifact, does not commit any `data/output` artifact, does not promote Gate 8 or Gate 9, does not call Boltz / AF3 / Chai, does not rerun enrichment or contrast, and does not make a biological claim.

After this PR, the next step should be `add_first_controlled_downstream_use_path_for_one_row_ready_artifact`, the first controlled downstream use path for the one-row ready artifact, not another transition approval.

## G3SX30 one-row readiness/preflight transition path decision

`data/input/g3sx30_one_row_readiness_preflight_transition_decisions.csv` records the final decision before the actual one-row G3SX30 readiness/preflight transition/check.

The decision sources `data/input/g3sx30_one_row_local_embedding_preflight_check_results.csv#1`, with `source_check_name=g3sx30_one_row_local_embedding_preflight_check`, `source_check_status=local_preflight_pass`, `source_embedding_shape=492x960`, `source_embedding_dtype=float32`, `source_embedding_finite=true`, `source_sequence_length_matches=true`, `source_local_runtime_embedding_tracked=false`, and `source_local_runtime_embedding_committed=false`.

The decision is `decision=approve_one_row_readiness_preflight_transition_path`, with `approved_for_next_transition_step=true`, `ready_for_preflight=false`, `gate8_promoted=false`, `gate9_promoted=false`, `biological_claim_made=false`, and `allowed_next_action=run_one_row_g3sx30_readiness_preflight_transition`.

This is the final decision PR before the actual transition/check. The anti-loop fields are `next_pr_must_be_actual_transition_check=true` and `no_additional_decision_before_transition=true`, with `next_required_pr_title=Run one-row G3SX30 readiness/preflight transition`. After this PR, the next PR must run the one-row G3SX30 readiness/preflight transition. Do not add another decision, review, scaffold, or binding layer before that.

This decision does not make a Biohub / ESMC call, does not rerun live embedding, does not generate a new embedding, does not commit the generated `.npy` artifact, does not commit any `data/output` artifact, does not commit external FASTA, live log, validation JSON, or local preflight JSON artifacts, does not promote `ready_for_preflight`, does not promote Gate 8 or Gate 9, does not call Boltz / AF3 / Chai, does not rerun enrichment or contrast, and does not make a biological claim.
...
The first TP53/MDM2 embedding-based NEGATOME control attempt is recorded in
`data/input/tp53_mdm2_first_embedding_based_negatome_control_results.csv#1`
and documented in
`docs/tp53_mdm2_first_embedding_based_negatome_control_result.md`.

This is a result-bearing checked blocker row. The local read-only audit
confirmed that the existing runtime is available. The ignored human `Q00987`
embedding and elephant `G3SX30` embedding are available with shapes `491x960`
and `492x960`.
...
The exact expected lookup key is
...
The audit read ignored `.npy` artifacts for validation. This PR writes or commits no `.npy` artifact and commits no `data/output` artifact. It performs
no automatic Biohub / ESMC call, no new embedding generation, no interface
scoring, no elephant interface scoring, no Boltz / AF3 / Chai call, no Gate 7
entry, no Gate 8 or Gate 9 promotion, and no biological claim.
...
TP53/MDM2 preflight now emits a generic strict panel summary, while the actual G3UAZ0 control ratio remains pending Gate 6 result integration.
...
One explicitly authorized Biohub / ESMC call generated the reviewed local
G3UAZ0 embedding. The artifact passed the `364x960`, `float32`, and finiteness
contracts and remained ignored, untracked, unstaged, and uncommitted.
...
The ratio is a technical interface-to-non-interface enrichment of absolute
cross-species changes in embedding coupling to the G3UAZ0 partner pool. It
does not establish binding, non-binding, binding strength, functional
...
The next result-bearing action is to integrate the actual ratio into the
